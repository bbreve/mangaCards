	...
	class AmazonWrapper{
		...
		public function execute()
		{
			...
			for($i = 1; $i<=$pages; $i++)
			{
				...
				foreach($products_rows as $product_row)
				{
					...
					{
						$plat = $xmlPageXPath -> query('.//a[contains(@class, "a-link-normal")]/h3[not(contains(text(), "sconosciuta")) and not(contains(text(), "sistema"))]/@href', $product_row);
						foreach($plat as $pl)
						{
							extractInPage($pl->nodeValue);	
						}
					}
				}
			}
		...
		function extractInPage($node)
		{
			$doc = new DomDocument;
			$doc->loadHTMLFile($node);
			...
			$producer = $xpath->query('//*[name()="a" and @id="brand"]/text()')->item(0)->textContent;
			...
			if(stristr($title, urldecode($query_product)) && $price != "€" )
			{
				...
				if(count($number[0]) == 0)
				{
					$this->appendArray($title, $node, $price, $image, $producer, $platform, 30000);
				}
				else
				{
